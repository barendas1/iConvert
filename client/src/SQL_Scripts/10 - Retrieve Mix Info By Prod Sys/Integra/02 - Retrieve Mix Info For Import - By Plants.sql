--> This SQL Query retrieves the Mixes in the specified Plants.
...
Begin
    ...
    (
	    ...
	    BatchPanelCode, MixClassNames, MixUsage,
    ...
    Select  MixData.PlantCode,
            ...
            MixData.BatchPanelCode,
            MixData.MixClassCode,
            MixData.MixUsageDescription,
            Case
            ...
            End As Strength,
            Case 
                When Isnull(MixData.AirContent, -1.0) < 0.0 Or Isnull(MixData.AirContent, -1.0) > 80.0
                Then Null
                Else MixData.AirContent
            End As AirContent,
            Null As MinAirContent,
            Null As MaxAirContent,
            Case
                When Isnull(MixData.DesignSlump, -1.0) <= 0.1
                Then Null
                Else Cast(MixData.DesignSlump As Float)
            End As Slump,
            Case
                When Isnull(MixData.MinSlump, -1.0) < 0.0 Or Isnull(MixData.MinSlump, -1.0) <= 0.1 And Isnull(MixData.MaxSlump, -1.0) <= 0.1
                Then Null
                Else MixData.MinSlump
            End As MinSlump,
            Case
                When Isnull(MixData.MaxSlump, -1.0) <= 0.1 Or Isnull(MixData.MinSlump, -1.0) <= 0.1 And Isnull(MixData.MaxSlump, -1.0) <= 0.1
                Then Null
                Else MixData.MaxSlump
            End As MaxSlump,
