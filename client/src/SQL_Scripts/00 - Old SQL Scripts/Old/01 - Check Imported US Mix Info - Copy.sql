...
Select	Plants.Name,
		...
		Mix.MaxWaterQuantity * 0.26417205 As CurrentMaxWaterQuantity,
		MixInfo.MinWCRatio,
		MixInfo.MaxWCRatio,
		MixInfo.MinWCMRatio,
		MixInfo.MaxWCMRatio,
...
Where	Ltrim(Rtrim(Isnull(MixInfo.MixCode, ''))) <> Ltrim(Rtrim(Isnull(ItemName.Name, ''))) Or
		...
		Round(Isnull(MixInfo.MaximumWater, -1.0), 8) <> Round(Isnull(Mix.MaxWaterQuantity * 0.26417205, -1.0), 8) Or
		Isnull(MixInfo.SackContent, -1.0) > 0.0 And 
		Round(Isnull(MixInfo.SackContent, -1.0), 8) <> Round(Isnull(Mix.SackContent, -1.0), 8) Or
	    Isnull(MixInfo.Price, -1.0) > 0.0 And Isnull(MixInfo.PriceUnitName, '') = 'Cu Yards' And 
	    Round(Isnull(MixInfo.Price, -1.0) * 1.307950547, 8) <> Round(Isnull(Mix.COST5, -1.0), 8) Or
...
Where	Isnull(@CheckForMatchingRecipeQuantities, 0) = 0 And
		...
		Case
			...
			When MixRecipe.QuantityUnitName In ('LB', 'Pounds', 'EA', 'Each')
			Then Round(MaterialRecipe.QUANTITY * 2.204622476, 6)
			When MixRecipe.QuantityUnitName In ('OZ', 'LQ OZ', 'Ounces', 'Fluid Oz', 'BG', 'Bag', 'Bags', 'DS')
		...
		Case
			...
			When MixRecipe.QuantityUnitName In ('LB', 'Pounds', 'EA', 'Each')
			Then Round(MaterialRecipe.QUANTITY * 2.204622476, 6)
			When MixRecipe.QuantityUnitName In ('OZ', 'LQ OZ', 'Ounces', 'Fluid Oz', 'Bags', 'Bag', 'BG', 'DS')
			Then Round(MaterialRecipe.QUANTITY / (Material.SPECGR * 0.0651984837440621 * 0.45359240000781), 6)
			When MixRecipe.QuantityUnitName = 'KG'
			Then Round(MaterialRecipe.QUANTITY, 6)
			When MixRecipe.QuantityUnitName = 'GR'
