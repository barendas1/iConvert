        ...
        Case
			When	MixInfo.Slump Is Not null And
					(
						...
						Isnull(MixInfo.Slump, -1.0) > 50.0
					)
			Then 'The Slump is not a value greater than Zero up to 50.0.'
			Else ''
		End As [Status Message],
        Case
			When	MixInfo.MinSlump Is Not null And
					(
						...
						Isnull(MixInfo.MinSlump, -1.0) > 50.0
					)
			Then 'The Minimum Slump is not a value from 0.0 to 50.0.'
			Else ''
		End As [Status Message],
        Case
			When	MixInfo.MaxSlump Is Not null And
					(
						...
						Isnull(MixInfo.MaxSlump, -1.0) > 50.0
					)
			Then 'The Maximum Slump is not a value from 0.0 to 50.0.'
			Else ''
		End As [Status Message],
        Case
			When	MixInfo.MinSlump Is Not null And
...
Select  MixRecipe.PlantCode As [Plant Name],
		...
		End As [Status Message],
		Case
			When Isnull(MixRecipe.QuantityUnitName, '') Not In ('lb', 'ga', 'oz', 'Fluid Oz', 'L', 'LT', 'KG', 'ML', 'GAL', 'GL', 'LQ OZ', 'Ounces', 'oz/cwt CM', 'Pounds', 'Gallons', 'Cubic Yards', 'EA', 'Each', 'CC', 'GR')
			Then 'The Mix Recipe Quantity Units is not one of the following: lb, ga, oz, Fluid Oz, l, lt, kg, ml, GAL, LQ OZ, ounces, oz/cwt CM, pounds, gallons, Cubic Yards, Each, CC, GR'
			Else ''
		End As [Status Message]
...
Where	IsNull(MixRecipe.MaterialItemCode, '') = '' Or
		...
		Isnull(MixRecipe.QuantityUnitName, '') Not In ('lb', 'ga', 'oz', 'Fluid Oz', 'L', 'LT', 'KG', 'ML', 'GAL', 'GL', 'LQ OZ', 'Ounces', 'oz/cwt CM', 'Pounds', 'Gallons', 'Cubic Yards', 'EA', 'Each', 'CC', 'GR')
...
Select	MixInfo.PlantCode As [Plant Name], 
		MixInfo.MixCode As [Mix Name],
		'The Mix has Admixtures or Fibers but does not have Cements or Minerals.'
From Data_Import_RJ.dbo.TestImport0000_XML_MixInfo As MixInfo
Inner Join #MixIDList As MixIDList
