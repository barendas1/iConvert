...
Where	Ltrim(Rtrim(Isnull(MixInfo.MixCode, ''))) <> Ltrim(Rtrim(Isnull(ItemName.Name, ''))) Or
		...
		Ltrim(Rtrim(Isnull(MixInfo.MixShortDescription, ''))) <> Ltrim(Rtrim(Isnull(ItemMaster.ItemShortDescription, ''))) Or
		Isnull(MixInfo.AirContent, 1.5) <> Isnull(Mix.AIR, -1.0) Or
		Isnull(MixInfo.MinAirContent, -1.0) <> Isnull(AirContentSpec.MinAirContent, -1.0) Or
		Isnull(MixInfo.MaxAirContent, -1.0) <> Isnull(AirContentSpec.MaxAirContent, -1.0) Or
		(Round(Isnull(MixInfo.Slump, -1.0), 6) <= 12.0 And Round(Isnull(MixInfo.Slump, -1.0), 6) <= 12.0 And Round(Isnull(MixInfo.Slump, -1.0), 6) <= 12.0) And
		(
		...
		(Round(Isnull(MixInfo.Slump, -1.0), 6) > 12.0 Or Round(Isnull(MixInfo.Slump, -1.0), 6) > 12.0 Or Round(Isnull(MixInfo.Slump, -1.0), 6) > 12.0) And
		(
...
Inner Join dbo.Name As MixNameInfo
On MixNameInfo.NameID = Mix.NameID
Inner Join @CementitiousInfo As CementitiousInfo
...
Where	Round(IsNull(MixRecipe.Quantity, -1.0), 6) <>
		Case
			...
			When MixRecipe.QuantityUnitName In ('OZ', 'Ounces')
			...
			When MixRecipe.QuantityUnitName = 'L'
			Then Round(MaterialRecipe.QUANTITY / Material.SPECGR / 1.0, 6)
			When MixRecipe.QuantityUnitName = 'ML'
