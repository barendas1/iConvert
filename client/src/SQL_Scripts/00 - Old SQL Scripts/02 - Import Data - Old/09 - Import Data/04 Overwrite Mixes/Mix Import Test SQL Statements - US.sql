...
Select	Plants.Name,
		...
		MixInfo.MinSlump, 
		MixInfo.MaxSlump,
		Round(Mix.SLUMP / 25.4, 6) As MixSlump,
		SlumpSpec.MinSlump, 
		SlumpSpec.MaxSlump,
		Round(Mix.MixTargetSpread / 25.4, 6) As MixTargetSpread,
...
Where	Ltrim(Rtrim(Isnull(MixInfo.MixCode, ''))) <> Ltrim(Rtrim(Isnull(ItemName.Name, ''))) Or
		...
		Round(Isnull(MixInfo.AirContent, -1.0), 8) >= 0.0 And Round(Isnull(MixInfo.AirContent, 1.5), 8) <> Round(Isnull(Mix.AIR, -1.0), 8) Or
		Round(Isnull(MixInfo.MinAirContent, -1.0), 8) >= 0.0 And Round(Isnull(MixInfo.MinAirContent, -1.0), 8) <> Round(Isnull(AirContentSpec.MinAirContent, -1.0), 8) Or
		Round(Isnull(MixInfo.MaxAirContent, -1.0), 8) >= 0.0 And Round(Isnull(MixInfo.MaxAirContent, -1.0), 8) <> Round(Isnull(AirContentSpec.MaxAirContent, -1.0), 8) Or
		(Round(Isnull(MixInfo.Slump, -1.0), 6) >= 0.0 Or Round(Isnull(MixInfo.MinSlump, -1.0), 6) >= 0.0 Or Round(Isnull(MixInfo.MaxSlump, -1.0), 6) >= 0.0) And
		(Round(Isnull(MixInfo.Slump, -1.0), 6) <= 12.0 And Round(Isnull(MixInfo.MinSlump, -1.0), 6) <= 12.0 And Round(Isnull(MixInfo.MaxSlump, -1.0), 6) <= 12.0) And
		(
			Round(Isnull(MixInfo.Slump, -1.0), 6) <> Round(Isnull(Mix.SLUMP / 25.4, -1.0), 6) Or
			Isnull(MixInfo.MinSlump, -1.0) <> Isnull(SlumpSpec.MinSlump, -1.0) Or
			Isnull(MixInfo.MaxSlump, -1.0) <> Isnull(SlumpSpec.MaxSlump, -1.0)
		) Or
		(Round(Isnull(MixInfo.Slump, -1.0), 6) > 12.0 Or Round(Isnull(MixInfo.MinSlump, -1.0), 6) > 12.0 Or Round(Isnull(MixInfo.MaxSlump, -1.0), 6) > 12.0) And
		(
			Round(Isnull(MixInfo.Slump, -1.0), 6) <> Round(Isnull(Mix.MixTargetSpread / 25.4, -1.0), 6) Or
...
Select	Plants.Name,
		...
		--Round(MaterialRecipe.QUANTITY * 2.204622476, 6) As QuantityInLbs,
		--Round(MaterialRecipe.QUANTITY / (Material.SPECGR * 0.0651984837440621 * 0.45359240000781), 6) As QuantityInOz
From dbo.Plants As Plants
Inner Join dbo.Batch As Mix
On Mix.Plant_Link = Plants.PlantId
Inner Join dbo.Name As MixNameInfo
...
Select 'Mixes To Be Imported', Count(*) As RecipeCount
From Data_Import_RJ.dbo.TestImport0000_XML_MixInfo As MixInfo
Inner Join 
(
    Select  Plants.Name As PlantCode, MixName.Name As MixCode
    From dbo.Plants As Plants
    Inner Join dbo.BATCH As Mix
    On Mix.Plant_Link = Plants.PlantId
...
On  ExistingMix.PlantCode = MixInfo.PlantCode And
    ExistingMix.MixCode = MixInfo.MixCode

Select 'Datebase Mixes', Count(*) As RecipeCount
From dbo.Plants As Plants
Inner Join dbo.BATCH As Mix
On Mix.Plant_Link = Plants.PlantId
Inner Join dbo.Name As MixName
...
On  Plants.Name = ExistingMix.PlantCode And
    MixName.Name = ExistingMix.MixCode
Inner Join dbo.MaterialRecipe As Recipe
On Recipe.EntityID = Mix.BATCHIDENTIFIER

Select 'Mix Recipe Materials Not In Mix Recipes In Database' As StatusMessage, MixInfo.*
From Data_Import_RJ.dbo.TestImport0000_XML_MixInfo As MixInfo
Inner Join 
(
    Select  Plants.Name As PlantCode, MixName.Name As MixCode
...
Left Join 
(
    Select -1 As ID, Plants.Name As PlantCode, MixName.Name As MixCode, Isnull(MtrlItemName.Name, '') As MaterialItemCode
    From dbo.Plants As Plants
    Inner Join dbo.BATCH As Mix
    On Mix.Plant_Link = Plants.PlantId
    Inner Join dbo.Name As MixName
    ...
    Left Join dbo.ItemMaster As MtrlItemMaster
    On MtrlItemMaster.ItemMasterID = Material.ItemMasterID
    Left Join dbo.Name As MtrlItemName
    On MtrlItemName.NameID = MtrlItemMaster.NameID
    Group By Plants.Name, MixName.Name, MtrlItemName.Name	
) As ExistingRecipe
On  ExistingRecipe.PlantCode = MixInfo.PlantCode And
    ExistingRecipe.MixCode = MixInfo.MixCode And
    ExistingRecipe.MaterialItemCode = MixInfo.MaterialItemCode
Where ExistingRecipe.ID Is Null

Select 'Mix Recipe Materials Not In Imported Mix Recipe Table' As StatusMessage, Plants.Name, MixName.Name, MtrlItemName.Name
From dbo.Plants As Plants
Inner Join dbo.BATCH As Mix
On Mix.Plant_Link = Plants.PlantId
Inner Join dbo.Name As MixName
...
Left Join dbo.Name As MtrlItemName
On MtrlItemName.NameID = MtrlItemMaster.NameID
Left Join Data_Import_RJ.dbo.TestImport0000_XML_MixInfo As MixInfo
On  Plants.Name = MixInfo.PlantCode And
    MixName.Name = MixInfo.MixCode And
    MtrlItemName.Name = MixInfo.MaterialItemCode
