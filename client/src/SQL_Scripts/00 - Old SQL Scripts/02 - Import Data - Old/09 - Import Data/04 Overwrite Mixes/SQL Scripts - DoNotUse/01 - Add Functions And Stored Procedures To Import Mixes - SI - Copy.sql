...
Begin
    ...
    Begin try
		...
		Begin
		    If Exists(Select * From Data_Import_RJ.sys.objects As Objects Where Objects.name = 'ImportProd02_MixItemCategory')		    
		    Begin
		    	Print ''
		    	...
		    	From Data_Import_RJ.dbo.ImportProd02_MixItemCategory As ItemCategoryInfo		    	
...
Begin
    ...
    Begin try
		...
		If Exists (Select * From Sys.databases As Databases Where Databases.name = 'Data_Import_RJ')
		Begin
		    If	Exists(Select * From Data_Import_RJ.sys.objects Where Name = 'ImportProd02_MixInfo')
		    Begin
		    	...
		    	(
		    		...
		    		From dbo.MATERIAL As Material
		    		Where Isnull(Material.Inactive, 0) = 0
				...
				From Data_Import_RJ.dbo.ImportProd02_MixInfo As MixInfo
				...
				Where	MixInfo.AutoID In
						(
							...
							Left Join
							(
								Select -1 As ID, MixRecipe.PlantCode, MixRecipe.MixCode
								From Data_Import_RJ.dbo.ImportProd02_MixInfo As MixRecipe
								Inner Join #MaterialInfo As MaterialInfo
							...
							Where	IsNull(MixInfo.MixCode, '') <> '' And									
									...
									MissingMaterial.ID Is Null And
									(
										AdmixtureMix.ID Is Null Or
										AdmixtureMix.ID Is Not null And
										CementitiousMix.ID Is Not null
									)			
				...
				Inner Join Data_Import_RJ.dbo.ImportProd02_MixInfo As MixRecipe
		        ...
		        Exec dbo.Utility_Print ''
		        Exec dbo.Utility_Print 'Show Mixes That Have Not Been Saved'
				...
				Update Mix
				    Set Mix.AIR = Case when IsNull(MixInfo.AirContent, -1.0) >= 0.0 And Isnull(MixInfo.AirContent, -1.0) <= 90.0 Then MixInfo.AirContent Else Mix.AIR End,
				        Mix.SLUMP = Case when Isnull(MixInfo.Slump, -1.0) >= 0.0 Then MixInfo.Slump Else Mix.SLUMP End,
				        Mix.MixTargetSpread = Case when Isnull(MixInfo.Spread, -1.0) >= 0.0 Then MixInfo.Spread Else Mix.MixTargetSpread End,
