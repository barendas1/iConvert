...
Where	Ltrim(Rtrim(Isnull(MixInfo.MixCode, ''))) <> Ltrim(Rtrim(Isnull(ItemName.Name, ''))) Or
	    ...
	    dbo.Validation_StringValueIsTrue(Isnull(MixInfo.MixInactive, '')) <> Isnull(Mix.MixInactive, 0) Or
...
Where	Round(IsNull(MixRecipe.Quantity, -1.0), 6) <>
		Case
			...
			When MixRecipe.QuantityUnitName In ('OZ', 'LQ OZ', 'Ounces', 'Fluid Oz')
			...
			When MixRecipe.QuantityUnitName In ('Each', 'Bag', 'EA')
			Then Round(MaterialRecipe.QUANTITY, 6)
