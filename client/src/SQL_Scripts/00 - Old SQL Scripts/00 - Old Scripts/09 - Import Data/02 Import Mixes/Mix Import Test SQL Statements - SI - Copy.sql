...
Where	Ltrim(Rtrim(Isnull(MixInfo.MixCode, ''))) <> Ltrim(Rtrim(Isnull(ItemName.Name, ''))) Or
	    ...
	    dbo.Validation_StringValueIsTrue(Isnull(MixInfo.MixInactive, '')) <> Isnull(Mix.MixInactive, 0)
...
Where	Round(IsNull(MixRecipe.Quantity, -1.0), 6) <>
		Case
			...
			When MixRecipe.QuantityUnitName In ('L', 'LT', 'Liters')
			Then Round(MaterialRecipe.QUANTITY / Material.SPECGR / 1.0, 6)
			When MixRecipe.QuantityUnitName In ('ML', 'CC', 'Ml-liter')
