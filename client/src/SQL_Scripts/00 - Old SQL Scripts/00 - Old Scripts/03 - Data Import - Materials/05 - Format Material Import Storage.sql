...
Begin
    ...
    Begin
		Update MaterialInfo
            Set MaterialInfo.MaterialDate = 
			        Right('00' + Ltrim(Rtrim(Cast(Month(MaterialInfo.MaterialDate) As Nvarchar))), 2) + '/' +
			        Right('00' + Ltrim(Rtrim(Cast(Day(MaterialInfo.MaterialDate) As Nvarchar))), 2) + '/' +
			        Right('0000' + Ltrim(Rtrim(Cast(Year(MaterialInfo.MaterialDate) As Nvarchar))), 4) 
		From Data_Import_RJ.dbo.TestImport0000_MaterialInfo As MaterialInfo
		Where Isdate(MaterialInfo.MaterialDate) = 1 And Len(MaterialInfo.MaterialDate) <> 10
    End
End
Go
...
Begin
    ...
    Begin
		Update MaterialInfo
			Set MaterialInfo.IsLiquidAdmix =
			        Case
			            When Isnull(MaterialInfo.FamilyMaterialTypeName, '') = 'Admixture & Fiber' And dbo.Validation_StringValueIsTrue(MaterialInfo.IsLiquidAdmix) = 1
			            Then 'Yes'
			            Else 'No'
			        End
		From Data_Import_RJ.dbo.TestImport0000_MaterialInfo As MaterialInfo		
