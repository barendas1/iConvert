...
Begin
    ...
    Begin
    	...
    	Begin
            ...
            Update MixInfo
                Set MixInfo.Strength = Cast(Ltrim(Rtrim(Left(MixInfo.ItemCategory, Charindex('psi', MixInfo.ItemCategory) - 1))) As Float)
            From Data_Import_RJ.dbo.TestImport0000_MixInfo As MixInfo
            Where   MixInfo.Strength Is Null And
                    Case
                        When Charindex('psi', MixInfo.ItemCategory) < 1
                        Then 0
                        When    Len(Ltrim(Rtrim(Left(MixInfo.ItemCategory, Charindex('psi', MixInfo.ItemCategory) - 1)))) = 4 And
                                Isnumeric(Ltrim(Rtrim(Left(MixInfo.ItemCategory, Charindex('psi', MixInfo.ItemCategory) - 1)))) = 1
                        Then 1
                        Else 0
                    End = 1
        ...
        From Data_Import_RJ.dbo.TestImport0000_MixInfo As MixInfo
        Where Isnull(MixInfo.Strength, -1.0) <= 0.0001
